	...
	module.exports = new Class( {
		...
		, send: function(){
			var data = "", headers = {}, statusCode = 200;
			...
			Array.prototype.slice.call( arguments, 0 ).forEach( function( arg ){
				switch ( type( arg ) ){
					...
					default:
						throw new Error( "The argument supplied is invalid. Accepting Number as statusCode, Buffer or String for as data & object for headers!" ).setName( "InvalidArgumentException" );
				}
			}.bind( this ) );
			...
			if ( data.length > 0 && acceptEncoding && acceptEncoding.indexOf( "gzip" ) >= 0 ){
				...
				zlib.gzip( data, function( err, compressedData ){
					if ( err ) this.__send( data, headers, statusCode );
						...
						else this.__send( data, headers, statusCode );
					}
				}.bind( this ) );
			}
			else this.__send( data, headers, statusCode );
		...
		, write: function(){
			return this.__response.write.apply( this.__response, Array.prototype.slice.call( arguments, 0 ) );
		}
		, end: function(){
			return this.__response.end.apply( this.__response, Array.prototype.slice.call( arguments, 0 ) );
		}
		...
		, handleListener: function( evt, listener ){
			switch( evt ){
				...
				case "unpipe":
				case "close":
					this.__response.on( evt, listener );
			}
		}
	} );