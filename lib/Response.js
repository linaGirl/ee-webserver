	...
	module.exports = new Class( {
		...
		, send: function() {
			...
			if (this.isSent) {
				this.emit('error', new Error('Cannot send reponse, the response was already sent before. '+this._request.method+' request on '+this._request.pathname));
			}
			else {
					...
					zlib.gzip(data, function(err, compressedData) {
						if (err) this._send(data, headers, statusCode);
							...
							if (compressedData && compressedData.length > 0 && compressedData.length < data.length) {
								...
								this._send(compressedData, headers, statusCode);
							}
		...
		, _send: function(data, headers, statusCode) {
			...
			// emit send event
			this.emit('send', (statusCode || this.statusCode), this.request.method.toLowerCase() === "head" ? undefined : data);
